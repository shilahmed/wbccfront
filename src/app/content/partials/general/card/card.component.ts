...
export class CardComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (
      this.elHeaderTitle &&
      this.elHeaderTitle.nativeElement.children.length === 0 &&
    ...
    if (
      this.elHeaderTools &&
      this.elHeaderTools.nativeElement.children.length === 0
    ) {
      this.elFooter.nativeElement.style.display = "none";
    }
  ...
  toggleCollpase(event) {
    ...
    if (this.elContent.nativeElement.classList.contains("show")) {
      ...
      if (
        !target.classList.contains("ft-plus") &&
        !target.classList.contains("ft-minus")
      ) {
  ...
  toggleExpand(event) {
    ...
    if (this.elCard.nativeElement.classList.contains("card-fullscreen")) {
      this.elCard.nativeElement.classList.remove("card-fullscreen");
      if (
        !target.classList.contains("ft-maximize") &&
        !target.classList.contains("ft-minimize")
      ) {
        target = event.target.querySelector("i");
      }
